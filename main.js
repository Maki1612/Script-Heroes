...
async function addItem(itemObj) {
  ...
  if (itemObj.id === "bronze_coins" || itemObj.id === "silver_coins") {
    ...
    // Re-fetch Silver (in case it was just added)
...
async function loadGameById(id) {
  ...
  // Restore dynamic world state from save flags
  if (GAME.story.roadUnlocked) {
    if (!WORLD_DATA.locations["Town Square"].connections.includes("Willowvane Road")) {
      WORLD_DATA.locations["Town Square"].connections.push("Willowvane Road");
    }
    if (WORLD_DATA.locations["Willowvane Road"]) {
      WORLD_DATA.locations["Willowvane Road"].locked = false;
    }
  }
...
const ENEMY_DATA = {
  "forest_wolf": {
    name: "Starving Wolf",
    baseHp: 55, baseAtk: 9, baseDef: 3, baseSpd: 7,
    xpReward: 25,
    bronzeReward: 12,
    abilities: [
      { name: "Bite", mult: 1.2 },
...
const ROAD_ENCOUNTERS = {
  // Key = destination, value = { steps, encounterChance, pool (weighted), flavorTexts }
  "Greenwood": {
    steps: 1,
    encounterChance: 0.5,
  ...
  "Sural City": {
    steps: [3, 5], // random between 3 and 5
...
function updateQuickActions() {
  ...
  location.connections.forEach(targetLoc => {
    ...
    if (targetData && targetData.locked) return; // skip locked locations
  ...
  // Post-wolf dialogue — unlocks the road
    ...
    if (!GAME.story.roadUnlocked) {
      ...
      onComplete = () => {
        ...
        // Add Willowvane Road to Town Square connections
        if (!WORLD_DATA.locations["Town Square"].connections.includes("Willowvane Road")) {
          WORLD_DATA.locations["Town Square"].connections.push("Willowvane Road");
        }
        // Unlock the road location
        if (WORLD_DATA.locations["Willowvane Road"]) {
          WORLD_DATA.locations["Willowvane Road"].locked = false;
        }
        autoSave();
...
function travelTo(locName) {
  ...
  if (ROAD_ENCOUNTERS[target] && ROAD_ENCOUNTERS[target].steps > 0) {
    log(`<br><em>You set off towards ${target}...</em>`);
    startRoadTravel(target, ROAD_ENCOUNTERS[target]);
  } else {
...
function travelTo(locName) {
  ...
  const hasSteps = config && (
    (typeof config.steps === 'number' && config.steps > 0) ||
    (Array.isArray(config.steps) && config.steps.length > 0)
  );

  if (hasSteps) {
    log(`<br><em>You set off towards ${target}...</em>`);
    startRoadTravel(target, config);
  } else {
    completeTravel(target);
  }
}
...
function showClassSelection() {
  const CLASS_INFO = {
    ...
    "Mage": {
      ...
      stats: "HP 80 | ATK 15 | DEF 5 | SPD 7"
    },
    "Ranger": {
